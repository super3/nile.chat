...
const channel = (to, from) => `${Math.min(this.to, this.from)}-${Math.max(this.to, this.from)}`;
...
module.exports = class DirectMessage {
	...
	static async find(instance, userA, userB) {
		const ids = await redis.lrange(`${instance}:direct:${channel(userA, userB)}:messages`, 0, 100);
	...
	static async findDirects(instance, id, loadConversations = true) {
		const users = await redis.lrange(`${instance}:directs:${id}`);
