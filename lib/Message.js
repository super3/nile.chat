...
module.exports = class Message {
	...
	static async search(instance, query, results = 10) {
		const ids = await redis.sinter(...search.chunkify(query).map(chunk => `${instance}:messages-search:${chunk}`));

		return Promise.all(ids.slice(0, results).map(json => {
			const [ channelId, id ] = JSON.parse(json);

			return this.get(instance, channelId, id);
		}));
	}
};
